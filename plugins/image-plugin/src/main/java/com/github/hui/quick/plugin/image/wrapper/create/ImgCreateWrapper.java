...
public class ImgCreateWrapper {
    ...
    public static class Builder {
        ...
        private String borderSignText = "  by @salk";
        ...
        public Builder drawContent(String content) {
            ...
            switch (options.getDrawStyle()) {
                ...
                case VERTICAL_FIXED:
                    return drawFixedContent(content);
                default:
                    return this;
            }
        }
        ...
        protected Builder drawHorizontalContent(String content) {
            ...
            int lineNum = CalculateHelper.calLineNum(strs,
                options.getImgW() - options.getLeftPadding() - options.getRightPadding(), fontMetrics);
            ...
            if (result == null) { // 首次绘制内容时
                result = GraphicUtil.createImg(options.getImgW(),
                    Math.max(height + options.getTopPadding() + options.getBottomPadding(), BASE_ADD_H), null);
            } else if (result.getHeight() < contentH + height + options.getBottomPadding()) {
                // 超过原来图片高度的上限, 则需要扩充图片长度
                result = GraphicUtil.createImg(options.getImgW(),
                    result.getHeight() + Math.max(height + options.getBottomPadding(), BASE_ADD_H), result);
            }
        ...
        protected Builder drawFixedContent(String content) {
            String[] strs = StringUtils.split(content, "\n");
            if (strs.length == 0) { // empty line
                strs = new String[1];
                strs[0] = " ";
            ...
            if (result == null) { // 首次绘制内容时
                result = GraphicUtil.createImg(options.getImgW(), options.getImgH(), null);
            }
            if(contentH+height <=this.getOptions().getImgH()){
                // 绘制文字
                doDrawContent(strs, fontHeight);
            }
            contentH += height;
        ...
        protected void doDrawContent(String[] strs, int fontHeight) {
            ...
            int index = 0;
            for (String str : strs) {
                ImgDrawHelper.drawContent(g2d, str, contentH + (fontHeight + options.getLinePadding()) * (++index),
                    options);
            }
            g2d.dispose();
        }
        ...
        protected Builder drawVerticalLeftContent(String content) {
            ...
            int lineNum = CalculateHelper.calVerticalLineNum(strs,
                options.getImgH() - options.getBottomPadding() - options.getTopPadding(), fontMetrics);
            ...
            if (result == null) {
                result = GraphicUtil.createImg(
                    ...
                    options.getImgH(), null);
                g2d = GraphicUtil.getG2d(result);
            } else if (result.getWidth() < contentW + width + options.getRightPadding()) {
                // 超过原来图片宽度的上限, 则需要扩充图片长度
                result =
                    GraphicUtil.createImg(result.getWidth() + Math.max(width + options.getRightPadding(), BASE_ADD_H),
                        options.getImgH(), result);
                g2d = GraphicUtil.getG2d(result);
            }
        ...
        protected void doDrawVerticalLeftContent(String[] strs, int fontSize) {
            ...
            for (String str : strs) {
                ImgDrawHelper.drawVerticalContent(g2d, str,
                    contentW + (fontSize + options.getLinePadding()) * (index++), options);
            }
            g2d.dispose();
        }
        ...
        protected Builder drawVerticalRightContent(String content) {
            ...
            if (result == null) {
                ...
                result = GraphicUtil.createImg(newW, options.getImgH(), newW - result.getWidth(), 0, result);
                g2d = GraphicUtil.getG2d(result);
            }
        ...
        protected void doDrawVerticalRightContent(String[] strs, int fontSize) {
            ...
            for (String str : strs) {
                ImgDrawHelper.drawVerticalContent(g2d, str, offsetX - (fontSize + options.getLinePadding()) * (++index),
                    options);
            }
            g2d.dispose();
        }
        ...
        private Builder drawHorizontalImage(BufferedImage bufferedImage) {
            // 绘制图片的实际高度
            int bfImgH = bufferedImage.getWidth() > options.getImgW()
                ? bufferedImage.getHeight() * options.getImgW() / bufferedImage.getWidth() : bufferedImage.getHeight();

            if (result == null) {
                result = GraphicUtil.createImg(options.getImgW(),
                    Math.max(bfImgH + options.getBottomPadding() + options.getTopPadding(), BASE_ADD_H), null);
            } else if (result.getHeight() < contentH + bfImgH + options.getBottomPadding()) {
                // 超过阀值
                result =
                    GraphicUtil.createImg(options.getImgW(),
                        result.getHeight()
            ...
            // 更新实际高度
            int h = ImgDrawHelper.drawImage(result, bufferedImage, contentH, options);
            contentH += h + options.getLinePadding();
            return this;
        }

        private Builder drawVerticalImage(BufferedImage bufferedImage) {
            int padding = options.getDrawStyle() == ImgCreateOptions.DrawStyle.VERTICAL_RIGHT ? options.getLeftPadding()
                : options.getRightPadding();

            // 实际绘制图片的宽度
            int bfImgW = bufferedImage.getHeight() > options.getImgH()
                ? bufferedImage.getWidth() * options.getImgH() / bufferedImage.getHeight() : bufferedImage.getWidth();
            if (result == null) {
                result = GraphicUtil.createImg(
                    Math.max(bfImgW + options.getLeftPadding() + options.getRightPadding(), BASE_ADD_H),
                    options.getImgH(), null);
            } else if (result.getWidth() < contentW + bfImgW + padding) {
                int realW = result.getWidth()
                    + Math.max(bfImgW + options.getLeftPadding() + options.getRightPadding(), BASE_ADD_H);
                int offsetX =
                    options.getDrawStyle() == ImgCreateOptions.DrawStyle.VERTICAL_RIGHT ? realW - result.getWidth() : 0;
                result = GraphicUtil.createImg(realW, options.getImgH(), offsetX, 0, result);
            }
        ...
        protected BufferedImage createBg(Point point) {
            ...
            }else if(options.getDrawStyle()== ImgCreateOptions.DrawStyle.VERTICAL_FIXED){
                realW = options.getImgW();
            ...
            BufferedImage bf = new BufferedImage((leftPadding << 1) + realW, realH + topPadding + bottomPadding,
            ...
            if (border) {
                if (this.borderImage != null) {
                    g2d.drawImage(borderImage, 0, 0, realW + (leftPadding << 1), realH + topPadding + bottomPadding,
                        null);
                } else {
                ...
                // g2d.setFont(new Font(ImgCreateOptions.DEFAULT_FONT.getName(),
                // ImgCreateOptions.DEFAULT_FONT.getStyle(), fSize));
